-- PharmaCare Database Tables Only (for existing database)
-- Use this if you already have the "pharmacare" database created
-- IMPORTANT: Make sure "pharmacare" database is selected in phpMyAdmin before importing!
...
-- 8. Sale Item Table (Items in each sale)
...
-- 9. Payment Table (Payment tracking)
...
-- 10. Return Table (Product returns)
...
-- 11. Refund Table (Refund processing)
CREATE TABLE IF NOT EXISTS refund (
    ...
    notes TEXT,
    FOREIGN KEY (return_id) REFERENCES return_table(return_id) ON DELETE CASCADE,
    FOREIGN KEY (approval_by) REFERENCES user(user_id) ON DELETE SET NULL,
    INDEX idx_return_id (return_id),
    INDEX idx_approval_by (approval_by)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 12. Notification Table (System alerts)
